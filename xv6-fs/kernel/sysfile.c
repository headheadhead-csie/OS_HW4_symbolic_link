...
sys_open(void)
{
    ...
    if(ip->type != T_SYMLINK || ((ip->type == T_SYMLINK) && (omode & O_NOFOLLOW) )){
        if(ip->type == T_DEVICE){
            f->type = FD_DEVICE;
            f->major = ip->major;
        } else {
        ...
        char target[MAXPATH];
        int cnt;
        for(cnt = 0; cnt < 10; cnt++){ // prevent cycle
            r = readi(ip, 0, (uint64)target, 0, MAXPATH);
            iunlock(ip);

            if((ip = namei(target)) == 0 || r >= MAXPATH){
                // printf("target file doesnt' exist or PATH is too long\n");
                end_op();
                return -1;
            }
            ilock(ip);
...
sys_symlink(void)
{
    ...
    struct inode *dp, *ip; // directory pointer
    ...
    if((dp = nameiparent(path, sym_name)) == 0){
        end_op();
        return -1;
    }
    ilock(dp);
    if(dirlookup(dp, sym_name, 0) != 0){
    ...
    if(ip == 0){
        // printf("Fail to create inode\n");
        end_op();
        return -1;
    }
    
    // below code is copied from filewrite
    int max = ((MAXOPBLOCKS-1-1-2) / 2) * BSIZE;
    int i = 0, n = strlen(target)+1, r, off = 0;
    while(i < n){
        int n1 = n - i;
        if(n1 > max)
            n1 = max;
        if ((r = writei(ip, 0, (uint64)target + i, 0, n1)) > 0)
            off += r;
        if(r != n1){
            // error from writei
            // printf("writei error\n");
